...
// Librerías necesarias para dibujar el emoji en el icono de la barra de tareas
#include <QPixmap>
#include <QPainter>
#include <QIcon>
...
public:
    CorazonWidget(QWidget *parent = nullptr) : BaseGLWidget(parent), frame(0) {
        setWindowTitle("❤️ Corazón 3D romántico de LYNDS Project - F11 para pantalla completa");

        // Usamos resize para que permita cambiar a Fullscreen libremente adaptándose a tu monitor
        resize(1400, 900);

        // --- RENDERIZADO DEL EMOJI PARA EL ICONO DE LA BARRA DE TAREAS ---
        QPixmap pixmap(64, 64);               // Lienzo transparente de 64x64 píxeles
        pixmap.fill(Qt::transparent);

        QPainter painter(&pixmap);
        QFont font = painter.font();
        font.setPixelSize(52);                 // Tamaño óptimo para el emoji
        painter.setFont(font);

        painter.drawText(pixmap.rect(), Qt::AlignCenter, "❤️");
        painter.end();

        setWindowIcon(QIcon(pixmap));          // Aplica el emoji como icono oficial de la app
        // -----------------------------------------------------------------
...
protected:
    void initializeGL() override {
        ...
        // Niebla para efecto cinematográfico de profundidad
        glEnable(GL_FOG);
        ...
        // Iluminación avanzada combinada
        ...
        // Luz principal - roja cálida frontal
        ...
        // Luz secundaria - azul frío lateral para contraste de bordes
        ...
        // Luz trasera - halo dorado de fondo
        ...
        // Material ultra brillante y sedoso
    ...
    void paintGL() override {
        ...
        // Reset de transformaciones para renderizar las partículas orbitales
        glLoadIdentity();
        glTranslatef(camSway, camBob - 0.5f, -16.0f);
        glRotatef(rotationX, 1.0f, 0.0f, 0.0f);
        glRotatef(rotationY, 0.0f, 1.0f, 0.0f);
    ...
    void keyPressEvent(QKeyEvent *event) override {
        if (event->key() == Qt::Key_Escape) {
            if (isFullScreen()) {
                showNormal(); // Regresa a modo ventana desde Fullscreen
            } else {
                close();      // Si ya está en modo ventana, cierra la aplicación
            }
...
private:
    void generateHeartGeometry() {
        ...
        for (int i = 0; i <= segments; i++) {
            for (int j = 0; j <= rings; j++) {
                ...
                // Cálculo de vectores normales precisos
    ...
    void drawHeartAura() {
        ...
        for (int layer = 0; layer < 3; layer++) {
            ...
            glBegin(GL_TRIANGLES);
            for (size_t i = 0; i < indices.size(); i += 3) {
                int idx0 = indices[i]; int idx1 = indices[i + 1]; int idx2 = indices[i + 2];
                if (idx0 < (int)vertices.size() && idx1 < (int)vertices.size() && idx2 < (int)vertices.size()) {
                    glVertex3f(vertices[idx0].x, vertices[idx0].y, vertices[idx0].z);
    ...
    void drawInnerGlow() {
        ...
        for (int i = 0; i < numGlows; i++) {
            ...
            float x = r * cosf(angle); float y = r * sinf(angle);
    ...
    void drawFloatingRosePetals() {
        ...
        for (int i = 0; i < numPetals; i++) {
            ...
            float x = radius * cosf(angle); float y = radius * sinf(angle) + 0.3f * sinf(time * 0.7f);
    ...
    void drawTrails() {
        ...
        for (int i = 0; i < numTrails; i++) {
            ...
            float radius = 1.5f;
            float x = radius * cosf(t); float y = radius * sinf(t);
...
int main(int argc, char *argv[]) {
    ...
    printf("║                                                  ║\n");
    printf("║  Iniciando directamente en PANTALLA COMPLETA...  ║\n");
    printf("╚══════════════════════════════════════════════════╝\n");
    printf("\n");

    CorazonWidget widget;

    // Inicia directamente en pantalla completa usando la API nativa de Qt
